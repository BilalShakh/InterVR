...
{
    public class VoiceData : MonoBehaviour
    {
        ...
        private string realFinalText;
        private Text myText;
        private double hesitation;
        private List<double> avgUmList = new List<double> { };
            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        if (!res.final)
                            realFinalText = alt.transcript;

                        avgUmList.Add(ParseHesitation(realFinalText));
                        hesitation = findListAvg(avgUmList);
                        Log.Debug("Percent Umms in total: ", hesitation.ToString());
                    ...
                    if (res.keywords_result != null && res.keywords_result.keyword != null)
                    {
                        ...
                        {
                            Log.Debug("ExampleStreaming.OnRecognize()", "keyword: {0}, confidence: {1}, start time: {2}, end time: {3}", keyword.keyword, keyword.confidence, keyword.start_time, keyword.end_time);
                        }
                    }
        ...
        {
            myText = GameObject.Find("StatText").GetComponent<Text>();
            myText.text = hesitation.ToString();
        }

        private double findListAvg(List<double> lst)
        {
            double sum = 0;
        ...
        private double ParseHesitation(string str)
        {
            ...
            return umCounter != 0 ? ((double)umCounter / (double)wordCounter) * 100 : 0.0;
        }
